...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - --  Fuhrer Edition  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by Fuhrer Ginx (") + handleClr.Sprintf("@mrgretzky") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
